...
from src.db.silver.models import SilverWwwTextSearch1d
...
@asset(
    key_prefix=["bronze"],
    name="wb_www_text_search_1d",
    resource_defs={"postgres": postgres_resource},
    required_resource_keys={"postgres"},
    description="Собирает сырые ответы поисковой выдачи WB (3 страницы) в bronze.wb_www_text_search_1d "
                "с заполнением run_schedule_dttm/receive_dttm/business_dttm.",
)
async def bronze_wb_www_text_search_1d(context):
    ...
    # бизнес-период — предыдущий день 00:00 МСК
    business_dttm = datetime.combine((run_schedule_dttm.date() - timedelta(days=1)), time.min, tzinfo=MSK)

    # 1) Пары (keyword, company_id) за «вчера» от плановой даты
    target_date = business_dttm.date()  # это и есть вчера от плановой
    async with context.resources.postgres() as session:

        rows = await session.execute(text("""
            SELECT DISTINCT kws.keyword, kws.company_id
            FROM silver.wb_adv_keyword_stats_1d AS kws
            JOIN core.algo_states AS st ON kws.advert_id = st.campaign_id
            WHERE kws.date = :target_date
    ...
    context.log.info(f"[bronze_www_text_search_1d] start run_uuid={run_uuid} "
        ...
        for keyword, company_id in pairs:
            for page in pages:
                ...
                await session.execute(
                    text("""
                    INSERT INTO bronze.wb_www_text_search_1d (
    ...
    context.log.info("[bronze_www_text_search_1d] done")
...
@asset(
    deps=[bronze_wb_www_text_search_1d],
    key_prefix=["silver"],
    name="wb_www_text_search_1d",
    resource_defs={"postgres": postgres_resource},
    required_resource_keys={"postgres"},
    description="Преобразует bronze.wb_www_text_search_1d → silver.wb_www_text_search_1d "
                "(PK: business_dttm, keyword, nm_id).",
)
async def silver_wb_www_text_search_1d(context):
    run_uuid = context.run_id
    ...
    async with context.resources.postgres() as session:
        rows = await session.execute(text("""
            ...
            FROM bronze.wb_www_text_search_1d b
    ...
    if not to_insert:
        context.log.info("Silver wb_www_text_search_1d: данных нет, пропускаем.")
        return
    ...
    async with context.resources.postgres() as session:
        for i in range(0, len(to_insert), chunk_size):
            ...
            upsert = pg_insert(SilverWwwTextSearch1d).values(chunk)
            # новый PK
            upsert = upsert.on_conflict_do_nothing(
                index_elements=["business_dttm", "keyword", "nm_id"],
            )
            await session.execute(upsert)
            await session.commit()
            context.log.info(f"[silver_www_text_search_1d] inserted chunk #{i // chunk_size + 1} rows={len(chunk)}")

    context.log.info(f"Silver wb_www_text_search_1d: всего загружено {len(to_insert)} строк")
